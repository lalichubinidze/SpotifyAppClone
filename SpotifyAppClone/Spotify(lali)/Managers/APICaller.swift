...
final class APICaller {
    ...
    //MARK: - Category
    public func getCategories(completions: @escaping (Result<[Category], Error>) -> Void) {
        createRequest(
            ...
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                guard let data = data, error == nil else{
                    completions(.failure(APIError.faileedToGetData))
                    return
                }
                do {
                    ...
                    completions(.success(result.categories.items))
                }
                catch {
